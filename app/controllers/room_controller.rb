...
class RoomController < ApplicationController
  def show
    if !session[:current_user_key].nil? || !params[:user_key].nil?
      ...
      if params[:id]
        ...
        @single_room = Room.where(name: @room_name).first || Room.create_private_room([@user, @chatting_with],
                                                                                      @room_name)
        ...
        if @user.notification
          ...
          @notifications_from.each do |n|
            @new_notification_from.append(n) if n != @chatid.to_i
          end

          @user.notification = false if @new_notification_from.empty?
      ...
      if GeneralInfo.exists?(userKey: user_key_current)
        ...
        @login_user_true = session[:current_user_key] if !session[:current_user_key].nil? && params[:user_key].nil?
