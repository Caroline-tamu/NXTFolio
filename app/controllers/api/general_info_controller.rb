  ...
  class GeneralInfoController < ApplicationController
    def index
      ...
      user_info = users.map do |user|
        ...
        if user_activities
          average_minutes_used_last_30_days = UserActivityDetail.where('user_id = ? AND created_at >= ?', id,
                                                                       30.days.ago).average(:time_spent_on_website).to_i / 60
        ...
        updated_at = user.updated_at.strftime('%Y-%m-%d %H:%M:%S')
        {
          access_enabled:,
          app:,
          average_minutes_used_last_30_days:,
